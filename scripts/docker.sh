...
print(
f"POSTGRES_HOST={shell_quote(host)}"
)

print(
f"POSTGRES_PORT={shell_quote(str(port))}"
)

print(
f"POSTGRES_USER={shell_quote(user)}"
)

print(
f"POSTGRES_PASSWORD={shell_quote(password)}"
)

print(
f"POSTGRES_DB={shell_quote(database)}"
)

print(
f"POSTGRES_SSLMODE={shell_quote(sslmode)}"
)
...
# --------------------------------------------------------
#
# Never allow the CI database to be the production database
# when production host information is explicitly supplied.
#
# --------------------------------------------------------
...
print(
"Backend database configuration: OK"
)
...
web_url="[http://127.0.0.1:${WEB_PORT}/](http://127.0.0.1:${WEB_PORT}/)"
...
web_health_url=
"[http://127.0.0.1:${WEB_PORT}${WEB_HEALTH_PATH}](http://127.0.0.1:${WEB_PORT}${WEB_HEALTH_PATH})"
...
api_url=
"[http://127.0.0.1:${WEB_PORT}${HEALTH_PATH}](http://127.0.0.1:${WEB_PORT}${HEALTH_PATH})"
